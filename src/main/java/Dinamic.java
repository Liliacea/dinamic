        ...
        for (int k = 0; k <= count; k++) {
            for (int s = 0; s <= maxWeight; s++) {
                if (k == 0 || s == 0) {
                    ...
                    if (s >= weights[k - 1]) {  //предмет помещается
                        A[k][s] = Math.max(A[k - 1][s], A[k - 1][s - weights[k - 1]] + prices[k - 1]); //макс ст-ть
                    } else {
                        A[k][s] = A[k - 1][s]; //предмет не помещается и мы его не кладем в рюкзак
                    }
                }
            }
        }
